	...
	.get((req,res,next)=>{
		...
		if(req.query.str){
			...
			.then(orgs=>{
				if(orgs!==null){
					...
					orgs.forEach((org,index)=>{
						Object.keys(org).forEach(key=>{
							if(orgs[index][key]!==null){// Account for null endorsement fields
								orgs[index][key]=orgs[index][key].toString().toLowerCase();
							}
						});
					});
					// Filter results - this compensates scanning every field in order to remove duplicates.
					let filteredArray=[];
					orgs.forEach((org,index)=>{
						Object.keys(org).forEach(key=>{
							if(orgs[index][key]!==null){// Account for null endorsement fields
				...
				else if(orgs===null){return res.status(204).json('Empty database');}
			})
			.catch(next)
		}
		else return res.status(406).json('No query specified');
	});
